...
require_once 'configuracion/conexion.php';
    class Box_model {
        ...
        public function insertarBox($id=null, $nombre, $departamento){ 
            ...
            if($row['nombreBox'] > 0){
                $_SESSION['error_registro'] = "¡Vaya!, este box ya esta registrado 💔";
            }else{
        ...
        public function obtener_box($box){
            $sql = "SELECT d.*, c.nombre AS 'departamento'
                                ,	u.id_centro AS 'centro'
                    FROM box d
                    INNER JOIN departamento c ON d.id_departamento = c.id_departamento
                    INNER JOIN centro u ON c.id_centro = u.id_centro
                    WHERE c.id_departamento = $box";

            $resultado = $this->db->query($sql);
            $row = $resultado->fetch_assoc();
           return $row;
        }
        public function modificarBox($id, $nombre, $departamento){        
            $resultado = $this->db->query("UPDATE box SET  nombre='$nombre', id_departamento='$departamento' WHERE id_box = $id");
        }
